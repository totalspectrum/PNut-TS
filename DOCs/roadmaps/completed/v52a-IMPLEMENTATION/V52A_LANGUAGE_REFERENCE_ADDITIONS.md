...
**New:** `NEXT level` (continue Nth enclosing REPEAT)

Skips the remainder of the current iteration and continues the loop at the specified nesting level. Without a level, behaves as before (innermost loop).

**Syntax:**
```spin2
NEXT              ' continue innermost REPEAT (unchanged)
NEXT level        ' continue level-th enclosing REPEAT (new)
```

**Parameters:**
| Parameter | Type | Range | Description |
|-----------|------|-------|-------------|
| level | Integer | 1..15 | Which enclosing REPEAT to continue (1 = innermost) |

**Example:**
```spin2
PUB main() | x, y, z
  REPEAT x FROM 0 TO 9                 ' level 3
    REPEAT y FROM 0 TO 9               ' level 2
      REPEAT z FROM 0 TO 9             ' level 1
        IF (z == 5)
          NEXT                          ' continue z loop (level 1)
        IF (y == 3) AND (z == 0)
          NEXT 2                        ' continue y loop (level 2)
        IF (x == 7) AND (y == 0)
          NEXT 3                        ' continue x loop (level 3)
```
...
**New:** `QUIT level` (exit N enclosing REPEAT loops)

Exits one or more enclosing REPEAT loops. Without a level, behaves as before (exits innermost loop only).

**Syntax:**
```spin2
QUIT              ' exit innermost REPEAT (unchanged)
QUIT level        ' exit level enclosing REPEATs (new)
```

**Parameters:**
| Parameter | Type | Range | Description |
|-----------|------|-------|-------------|
| level | Integer | 1..15 | Number of enclosing REPEAT loops to exit |
...
PUB main() | x, y
  REPEAT x FROM 0 TO 99                ' outer loop
    REPEAT y FROM 0 TO 99              ' inner loop
      ...
      process(x, y)
  ' Execution continues here after QUIT 2
```

**Notes:**
- Level 1 is equivalent to plain `QUIT`
- `QUIT 2` exits two nested loops, `QUIT 3` exits three, etc.
...
| Spin2 Statement | `NEXT level` | Continue Nth enclosing REPEAT |
| Spin2 Statement | `QUIT level` | Exit N enclosing REPEAT loops |
