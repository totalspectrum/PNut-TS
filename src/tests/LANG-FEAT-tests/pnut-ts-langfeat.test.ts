...
// test lives in <rootDir>/src/tests/LANG-FEAT-tests
const testDirPath = path.resolve(__dirname, '../../../TEST/LANG-FEAT-tests');
...
describe('PNut_ts compiles FIELD operator tests correctly', () => {
  const fieldFiles = allFiles.filter((file) => path.basename(file).startsWith('field_'));
  fieldFiles.forEach((file) => {
    test(`Compile file: ${path.basename(file)}`, () => {
      runCompilationTest(file);
    });
  });
});

describe('PNut_ts compiles STRUCT tests correctly', () => {
  const structFiles = allFiles.filter((file) => path.basename(file).startsWith('struct_'));
