...
describe('PNut_ts generates correct resolver responses', () => {
  ...
  files.forEach((file) => {
    test(`Compile file: ${path.basename(file)}`, () => {
      ...
      const reportFSpec = path.join(testDirPath, `${basename}.resolv`);
      ...
      const reportDebugFSpec = path.join(testDirPath, `${basename}.resolv.txt`);
      // if the diagnostic file exists delete it before we start
      removeExistingFile(reportDebugFSpec);

      // Read the golden file
      const goldenFSpec = path.join(testDirPath, `${basename}.resolv.GOLD`);
      const goldenContentLines = fs.readFileSync(goldenFSpec, 'utf8').split('\n');
      const goldenDebugFSpec = path.join(testDirPath, `${basename}.resolv.GOLD.txt`);
      // if the diagnostic file exists delete it before we start
